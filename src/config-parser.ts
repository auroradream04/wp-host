...
export class ConfigParser {
  ...
  static async parseConfig(configPath: string): Promise<Config> {
    ...
    if (ext === ".json") {
      ...
      throw new Error(
        `Unsupported configuration file format: ${ext}. Supported formats: .json, .csv`,
      );
    }
  }
  ...
  private static async parseCsvConfig(configPath: string): Promise<Config> {
    try {
      ...
      const isComprehensiveFormat =
        headers.some(
          (h) => h.toLowerCase() === "mysql host" || h === "mysql_host",
        ) &&
        headers.some(
          (h) =>
            h.toLowerCase() === "wordpress admin password" ||
  ...
  private static parseComprehensiveCSV(
    ...
    for (const required of requiredFields) {
      if (!headerMap[required]) {
        throw new Error(
          `Missing required CSV header: ${required} (or its user-friendly equivalent)`,
        );
      }
    }
   ...
   */
  private static validateSiteConfig(
    site: SiteConfig,
    index: number,
    ...
    if (site.site_name && !/^[a-zA-Z0-9_-]+$/.test(site.site_name)) {
      errors.push({
        field: "site_name",
        message:
          "Site name can only contain letters, numbers, underscores, and hyphens",
        siteIndex: index,
      });
    }
   ...
   */
  private static validateMySQLConfig(
    mysql: MySQLConfig,
    errors: ValidationError[],
  ): void {
   ...
   */
  private static validateWordPressConfig(
    wordpress: WordPressConfig,
    errors: ValidationError[],
  ): void {
